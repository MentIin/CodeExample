...
namespace Assets.CodeBase.Infrastructure.Services.MapGeneration.NoiseMap
{
    public class NoiseMapGeneratorNew
    {
        private float[] _scaleNoiseMap;
        ...
        public float[] GenerateNoiseMap(int width, int height, int seed, float scale, int octaves, float persistence, float lacunarity, Vector2 offset)
        {
            ...
            _scaleNoiseMap = noiseMapGenerator.GenerateNoiseMap(width, height, seed + 1, 30, 4, 1, lacunarity, offset);
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    for (int i = 0; i < octaves; i++)
                    {
                        ...
                        localScale = (_scaleNoiseMap[x*y] + 14f) / 15f  * scale;
                        
                        
                        float xResult = (x - halfWidth) / localScale * frequency + octavesOffset[i].x * frequency;
                        float yResult = (y - halfHeight) / localScale * frequency + octavesOffset[i].y * frequency;
